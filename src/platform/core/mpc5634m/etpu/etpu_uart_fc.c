 ...
 * 2.1          R.Chovanec    27/Jul/05 Updated naming convention to UART_FC *
/**************************************************************************/
#include "etpu/etpu_util.h"          /* Utility routines for working eTPU */
#include "etpu/etpu_uart_fc.h"       /* eTPU UART API header */
...
/* get parameter RAM
   number of parameters passed from eTPU C code */
          pba = fs_etpu_malloc (FS_ETPU_UART_FC_NUM_PARMS);
    ...
    *(pba8 + (FS_ETPU_UART_FC_BITS_PER_WORD_OFFSET)) = bits_per_data_word;
    ...
    eTPU->CHAN[rx_channel].HSRR.R = FS_ETPU_UART_FC_RX_INIT;
...
/* get parameter RAM
    ...
    *(pba + ((FS_ETPU_UART_FC_TX_RX_DATA_OFFSET - 1) >> 2)) = 0x800000;
    ...
    *(pba8 + (FS_ETPU_UART_FC_BITS_PER_WORD_OFFSET)) = bits_per_data_word;
    ...
    /* write hsr to start channel running */
    eTPU->CHAN[tx_channel].HSRR.R = FS_ETPU_UART_FC_TX_INIT;


/* added new code for RTS channel which is (Rx_channel + 1)				*/
/******************************************************************************/
/******************************************************************************/
/* initialize RTS channel */
    if (eTPU->CHAN[(rx_channel+1)].CR.B.CPBA == 0)
      {
/* get parameter RAM
        number of parameters passed from eTPU C code */
          pba = fs_etpu_malloc (FS_ETPU_UART_FC_NUM_PARMS);
      ...
      {                         /*set pba to what is in the CR register */
          pba = fs_etpu_data_ram ((rx_channel + 1));
      }

    pba8 = (uint8_t *) pba;

    /* write channel configuration register */
    eTPU->CHAN[(rx_channel + 1)].CR.R = (priority << 28) +
        (FS_ETPU_UART_FC_TABLE_SELECT << 24) +
        (FS_ETPU_UART_FC_FUNCTION_NUMBER << 16) +
        (((uint32_t) pba - fs_etpu_data_ram_start) >> 3);

    /* write hsr to start channel running */
    eTPU->CHAN[(rx_channel + 1)].HSRR.R = FS_ETPU_UART_FC_RTS_INIT;
...
/* write the transmit data register	*/
    ...
    *(pba + ((FS_ETPU_UART_FC_TX_RX_DATA_OFFSET - 1) >> 2)) = transmit_data;
...
int32_t fs_etpu_uart_read_receive_data (uint8_t rx_channel, uint8_t * rx_error)
{
    ...
    *rx_error = *(pba8 + FS_ETPU_UART_FC_RX_ERROR_OFFSET);

   /* get received data  */
    receive_data = *(pba + ((FS_ETPU_UART_FC_TX_RX_DATA_OFFSET - 1) >> 2));

/* write hsr to assert RTS control line after data has been read */
    eTPU->CHAN[(rx_channel + 1)].HSRR.R =  FS_ETPU_UART_FC_READ_DATA;

    /*Mask upper byte and right justify the data */
    receive_data &= 0x00ffffff;
    return (receive_data >>
            (23 - *(pba8 + FS_ETPU_UART_FC_BITS_PER_WORD_OFFSET)));

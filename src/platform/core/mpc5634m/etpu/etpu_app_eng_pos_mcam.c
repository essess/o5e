/**************************************************************************
 * FILE NAME: $RCSfile: etpu_app_eng_pos_mcam.c,v $                       *
 *                                           COPYRIGHT (c) FREESCALE 2013 *
 *                                                  All Rights Reserved   *
 * DESCRIPTION:                                                           *
 * This file contains the eTPU Engine Position (eng_pos_mcam) API.             *
 *========================================================================*
 * ORIGINAL AUTHOR: Milan Brejl (r54529)                                  *
 * $Log: etpu_app_eng_pos_mcam.c,v $
 * Revision 1.0  2013/01/25 09:53:27  r54529
 * Initial revision. Based on etpu_app_eng_pos.c.
 *                                                                        *
 **************************************************************************/
#include "etpu/etpu_util.h"		    /* Utility routines for working with the eTPU */
#include "etpu/etpu_app_eng_pos_mcam.h"		  /* eTPU Engine Position API defines */
...
uint8_t g_crank_channel, g_mcam_channel;
/******************************************************************************
FUNCTION     : fs_etpu_app_eng_pos_mcam_init
PURPOSE      : To initialize 2 eTPU channels to for the synchronisation
               functions.
INPUTS NOTES : This function has the following parameters:
          mcam_channel - This is the MCAM channel number.
          ...
          cam_priority - This is the priority to assign to the cam channel.
                         ...
                         FS_ETPU_MCAM_PRIORITY_HIGH, FS_ETPU_PRIORITY_MCAM_MIDDLE
                         or FS_ETPU_MCAM_PRIORITY_LOW.
     cam_edge_polarity - The polarity if the CAM edge. Can be
                         FS_ETPU_MCAM_FM0_RISING_EDGE or
                         FS_ETPU_MCAM_FM0_FALLING_EDGE
...
{
    uint32_t *cam_pba;	 /* parameter base address for MCAM channel */
    ...
    uint32_t angle_start_count_tcr2; /* corresponds to either 360 or 720 */

  int32_t error_code = 0; /* returned value from etpu API functions */

  g_crank_channel = crank_channel;
  g_mcam_channel = mcam_channel;

	/* Disable MCAM channel to assign function safely */
	fs_etpu_disable( mcam_channel );
	/* Disable CRANK channel to assign function safely */
	fs_etpu_disable( crank_channel );

    if (eTPU->CHAN[mcam_channel].CR.B.CPBA == 0 )
    {
    	/* get parameter RAM for
    	number of parameters passed from eTPU C code */
    	cam_pba = fs_etpu_malloc(FS_ETPU_MCAM_NUM_PARMS );
    ...
    {
    	cam_pba=fs_etpu_data_ram(mcam_channel);
    }
	...
	eTPU->CHAN[mcam_channel].CR.B.CPBA = (((uint32_t)cam_pba -  fs_etpu_data_ram_start)>>3);
  ...
  if (cam_angle_window_start_tcr2_count > (tcr2_counts_per_engine_cycle>>1))
  {
    angle_start_count_tcr2 = tcr2_counts_per_engine_cycle;  /* 720 */
  }
  else
  {
    angle_start_count_tcr2 = tcr2_counts_per_engine_cycle>>1;  /* 360 */
  }

    crank_cam_state_ptr = cam_pba
                    -  (uint32_t) ((uint32_t *) ((fs_etpu_data_ram_start) / 4))
                   + (FS_ETPU_MCAM_STATE_OFFSET >>2);

	/* write parameters to data memory for MCAM */

   *(cam_pba + ((FS_ETPU_MCAM_TCR2_COUNTS_PER_ENGINE_CYCLE_OFFSET - 1)>>2)) = tcr2_counts_per_engine_cycle;
   *(cam_pba + ((FS_ETPU_MCAM_ANGLE_WINDOW_START_NEW_OFFSET - 1)>>2)) = cam_angle_window_start_tcr2_count;
   *(cam_pba + ((FS_ETPU_MCAM_ANGLE_WINDOW_WIDTH_NEW_OFFSET - 1)>>2)) = cam_angle_window_width_tcr2_count;
   *(cam_pba + ((FS_ETPU_MCAM_ANGLE_START_COUNT_OFFSET - 1)>>2)) = angle_start_count_tcr2;

   *((uint8_t *)cam_pba + FS_ETPU_MCAM_STATE_OFFSET) = (uint8_t)FS_ETPU_MCAM_SYNC_SEEK_OR_START_ANGLE;
   *((uint8_t *)cam_pba + FS_ETPU_MCAM_ERROR_STATUS_OFFSET) = (uint8_t)FS_ETPU_MCAM_NO_ERROR;
  ...
  *(crank_pba + ((FS_ETPU_CRANK_ANGLE_START_COUNT_OFFSET - 1)>>2)) = angle_start_count_tcr2;
  ...
  *((uint8_t *)crank_pba + FS_ETPU_CRANK_BLANK_TIME_EXPIRED_FLAG_OFFSET) = (uint8_t)0;
  ...
  fs_etpu_set_global_24(FS_ETPU_ENG_POS_SYNC_STATUS_GLOBAL_OFFSET, FS_ETPU_ENG_POS_SEEK);
	...
	eTPU->CHAN[mcam_channel].SCR.R = cam_edge_polarity;
    ...
    eTPU->CHAN[mcam_channel].HSRR.R = FS_ETPU_MCAM_INIT;

 /* write channel configuration registers */
	eTPU->CHAN[mcam_channel].CR.R =
	      (cam_priority << 28) +
				(FS_ETPU_MCAM_TABLE_SELECT << 24) +
	      (FS_ETPU_MCAM_FUNCTION_NUMBER << 16) +
                                               ...
                                               uint32_t cam_angle_window_width)
{
  ...
  cam_angle_window_width_tcr2_count = ((long long)cam_angle_window_width * tcr2_counts_per_engine_cycle)/(long long)72000;
	...
	fs_etpu_set_chan_local_24( g_mcam_channel,
	                           FS_ETPU_MCAM_ANGLE_WINDOW_WIDTH_HOST_OFFSET,
													 	 cam_angle_window_width_tcr2_count);

	fs_etpu_set_chan_local_24( g_mcam_channel,
	                           FS_ETPU_MCAM_ANGLE_WINDOW_START_HOST_OFFSET,
													 	 cam_angle_window_start_tcr2_count );

   eTPU->CHAN[g_mcam_channel].HSRR.R = FS_ETPU_MCAM_UPDATE_WINDOW;
...
{
	return(fs_etpu_get_chan_local_8( g_mcam_channel,
	                                 FS_ETPU_MCAM_ERROR_STATUS_OFFSET));
}
...
uint32_t fs_etpu_eng_pos_mcam_get_cam_edge_angle()
{
  ...
  cam_edge_angle = ((long long)fs_etpu_get_chan_local_24( g_mcam_channel,
	                                 FS_ETPU_MCAM_EDGE_ANGLE_OFFSET) * 72000)/(long long)tcr2_counts_per_engine_cycle;
...
{
	  fs_etpu_set_chan_local_8( g_mcam_channel,
	                            FS_ETPU_MCAM_ERROR_STATUS_OFFSET,
														  FS_ETPU_MCAM_NO_ERROR );
    return(0);
}

int32_t fs_etpu_eng_pos_mcam_get_engine_cycle_0_tcr2_count()
{
	  return(fs_etpu_get_chan_local_24( g_mcam_channel,
	                            FS_ETPU_MCAM_ENGINE_CYCLE_0_COUNT_OFFSET));
}
